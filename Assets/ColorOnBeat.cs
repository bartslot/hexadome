...
public class ColorOnBeat : MonoBehaviour
{
    ...
    public Color _color;
    public string _colorProperty = "_EmissionColor";
    ...
    void CheckBeat()
    {
        _beatCountFull = BeatManager._beatCountFull & 4;
        for (int i = 0; i < _onBeatD8.Length; i++)
        {
            if (BeatManager._beatD8 && _beatCountFull == _onFullBeat && BeatManager._beatCountD8 % 8 == _onBeatD8[i])
            {
                Colorize();
            }
        }
